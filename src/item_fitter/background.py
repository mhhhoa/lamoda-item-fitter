...
def _poly_basis(h: int, w: int, degree: int) -> np.ndarray:
    ...
    Кэшируется: на кадре фит вызывается дважды, а базис зависит только от размера
    и степени. На 1500x2000 это экономит построение массива на 72 МБ.
...
def white_roll_off(
    srgb: np.ndarray,
    knee_low: float = 0.985,
    knee_high: float = 0.998,
    ...
    Пороги намеренно высокие, а сглаживание по умолчанию выключено.

    Раньше решение о дотяжке смягчалось гауссом, чтобы шум JPEG не оставлял в углах
    254 вместо 255. Ради этих единиц побеление расползалось на пару пикселей вокруг
    всего светлого: белый реквизит в кадре терял форму, а светлая кожа выбеливалась.
    Точное значение в углу роли не играет — важно, что фон белый на вид, — поэтому
    смягчения больше нет, а порог поднят выше светлой кожи и белого реквизита.

    smooth_sigma оставлен параметром на случай, если понадобится вернуть прежнее
    поведение, но по умолчанию не применяется.
