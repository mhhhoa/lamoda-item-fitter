...
class Preset:
    ...
    fill: float = 1.0
    #: на сколько пикселей низ товара заходит НИЖЕ линии отступа.
    #:
    #: Ламода требует, чтобы товар «касался нижнего отступа», и её модерация
    #: понимает это строго: размещение ровно на линии она бракует. Замер 10
    #: вписанных фото, прошедших модерацию, показал, что товар у них заходит
    #: в зону отступа — нижнее поле 312…361 при медиане 347, то есть в среднем
    #: на 13 px ниже линии. Ровно 360 оказалось единственным краем
    #: распределения и на практике читается их проверкой как зазор.
    bottom_overshoot: int = 12
