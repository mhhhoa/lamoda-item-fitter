...
def default_workers() -> int:
    """Один рабочий процесс: надёжность здесь важнее скорости.

    Параллельные процессы давали выигрыш в минуты на больших пачках, но
    каждый из них — отдельная копия numpy, scipy и Pillow плюс полноразмерный
    кадр в памяти. На рабочем ноутбуке с открытым браузером три таких копии
    упираются в предел оперативной памяти, и дальше решение принимает уже не
    программа, а система: кого-то она убивает без объяснений. Пачка, которая
    считается вдвое дольше, но доходит до конца, полезнее быстрой и
    оборванной.
    """
    return 1
