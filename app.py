...
Кроме обычного запуска умеет два ключа:

* `--selftest ФАЙЛ` — собранный exe проверяет сам себя (пресет на месте,
  геометрия считается, Qt поднимается) и пишет отчёт. Это единственный
  способ убедиться, что упаковка не потеряла зависимость: из CI собранное
  окно руками не потрогать.
* `--create-shortcut` — кладёт ярлык на рабочий стол и показывает результат.
  С этим ключом программу зовёт лежащий рядом «Создать ярлык на рабочем
  столе.bat»; отдельного установщика нет намеренно, второй неподписанный
  exe означал бы второе окно SmartScreen у каждого коллеги.
"""
...
def main() -> int:
    if "--create-shortcut" in sys.argv:
        from lamoda_item_fitter.gui.shortcut_ui import run_installer
